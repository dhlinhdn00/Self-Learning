# 📚 Cheat Sheets & Learning Links

> Curated resources for quick learning in AI/ML/DE, CS, algorithms, OCR, and more. Grouped by topics for easier navigation.

## Table of Contents
- [🎓 Computer Science](#-computer-science)
- [🧠 LLM & GenAI](#-llm--genai)
- [🤖 Reinforcement Learning](#-reinforcement-learning)
- [🖼️ OCR / CV](#️-ocr--cv)
- [📊 Basic Machine Learning](#-basic-machine-learning)
- [🧰 Tooling / Books / Courses](#-tooling--books--courses)
- [🧱 Data Engineering](#-data-engineering)
- [🧮 Algorithms & Data Structures](#-algorithms--data-structures)

---

## 🎓 Computer Science
- Computer Science courses with video lectures — <https://github.com/Developer-Y/cs-video-courses>

## 🧠 LLM & GenAI
- AI Engineering Toolkit — <https://github.com/Sumanth077/ai-engineering-toolkit>  
- Awesome LLM Apps — <https://github.com/Shubhamsaboo/awesome-llm-apps>  
- LLM Engineer's Handbook — <https://github.com/mlabonne/llm-course>  
- Hands-On LLM — <https://github.com/HandsOnLLM/Hands-On-Large-Language-Models>  
- AI Agents For Beginners — <https://github.com/microsoft/ai-agents-for-beginners>  
- GenAI Agents — <https://github.com/NirDiamant/GenAI_Agents>  
- Prompt Engineering Guide — <https://github.com/dair-ai/Prompt-Engineering-Guide>  
- Hands-On AI Engineering — <https://github.com/Sumanth077/Hands-On-AI-Engineering>  
- Awesome Generative AI Guide — <https://github.com/aishwaryanr/awesome-generative-ai-guide>

## 🤖 Reinforcement Learning
- Deep RL Course — <https://huggingface.co/learn/deep-rl-course> • Repo: <https://github.com/huggingface/deep-rl-class>  
- AI Toolbox for Reinforcement Learning — <https://github.com/Svalorzen/AI-Toolbox>

## 🖼️ OCR / CV
- MediaPipe — <https://github.com/google-ai-edge/mediapipe>
- Yolo — <https://github.com/ultralytics/ultralytics>
- rPPG Toolbox — <https://github.com/ubicomplab/rPPG-Toolbox>  
- Vietnamese OCR Toolbox — <https://github.com/kaylode/vietnamese-ocr-toolbox>  
- MM OCR — <https://github.com/open-mmlab/mmocr>

## 📊 Basic Machine Learning
- Dive Into Deep Learning (d2l-ai) — <https://github.com/d2l-ai>  
- Designing Machine Learning Systems — <https://github.com/chiphuyen/dmls-book>  
- ML For Beginners — <https://github.com/microsoft/ML-For-Beginners>  
- Basic Machine Learning (Vietnamese) — <https://machinelearningcoban.com/>  
- Made With ML — <https://github.com/GokuMohandas/Made-With-ML>

## 🧰 Tooling / Books / Courses
- Awesome AI Tools - <https://github.com/mahseema/awesome-ai-tools>
- AI Engineering Toolkit — <https://github.com/Sumanth077/ai-engineering-toolkit>  
- Hands-On AI Engineering — <https://github.com/Sumanth077/Hands-On-AI-Engineering>

## 🧱 Data Engineering
- Awesome Open Source Data Engineering — <https://github.com/pracdata/awesome-open-source-data-engineering>  
- Awesome Data Engineering — <https://github.com/igorbarinov/awesome-data-engineering>  
- Data Engineering Roadmap — <https://github.com/datastacktv/data-engineer-roadmap>  
- Data Engineering ZoomCamp — GitHub: <https://github.com/DataTalksClub/data-engineering-zoomcamp> • Blog: <https://datatalks.club/blog/data-engineering-zoomcamp.html>  
- Confluent Examples (Kafka, ksqlDB, Streams) — <https://github.com/confluentinc/examples>  
- Apache Flink Training Exercises — *(same link as Confluent examples, kept from original list)* — <https://github.com/confluentinc/examples>  
- Spark — *The Definitive Guide* notebooks — <https://github.com/databricks/Spark-The-Definitive-Guide>  
- dbt — Jaffle Shop — <https://github.com/dbt-labs/jaffle-shop>  
- Open Metadata — <https://github.com/open-metadata/OpenMetadata>  
- Great Expectations — <https://github.com/great-expectations/great_expectations>

## 🧮 Algorithms & Data Structures
- Algorithm and Data Structures For Interview — <https://github.com/MAZHARMIK/Interview_DS_Algo>

---
